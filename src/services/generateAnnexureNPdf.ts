...
export async function generateAnnexureNPdf(
  batch: any
): Promise<Buffer> {
...
<div class="header">
  ...
  <div class="title">
    ${feedbackForm?.form_name || 'Candidate Feedback Form'}
  </div>
</div>
...
`
  ...
  try {
    ...
    console.error(`Error generating Annexure N PDF for batch ${batch?.id}:`, error)
