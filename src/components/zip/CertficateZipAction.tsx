...
const CertificateZipAction = ({ batchId }: { batchId: number }) => {
  ...
  const handleGenerate = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/batches/${batchId}/zip/certificate`,
        {
          method: 'POST',
  ...
  const fetchExistingJob = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/batches/${batchId}/zip/job?type=generate_certificate`
      )
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/batches/${batchId}/zip/download?type=generate_certificate`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <div className='flex items-center gap-2'>
      <Tooltip title={job?.status === 'completed' ? 'Regenerate Certificates' : 'Generate Certificates'}>
        <Button
          variant="contained"
...
export default CertificateZipAction
