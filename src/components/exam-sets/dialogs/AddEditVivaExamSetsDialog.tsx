...
const AddEditVivaExamSetsDialog = ({ open, examSetId, handleClose, updateExamSetsList }: AddQPDialogProps) => {
  ...
  useEffect(() => {
    if(totalVivaMarks && sumOfSelectedQuestionsMarks){
      ...
      const isEqual = totalVivaMarks === sumOfSelectedQuestionsMarks;

      if(isEqual){
        clearErrors(['totalQuestions']);
      } else {
        setError('totalQuestions', {type: 'custom', message: `The sum of selected questions marks must equal to Total Viva Marks. ${totalVivaMarks}`});
      }

    }
  }, [totalVivaMarks, sumOfSelectedQuestionsMarks])
  ...
  return (
    ...
    >
      ...
      <DialogTitle variant='h4' className='flex gap-2 flex-col text-center sm:pbs-16 sm:pbe-6 sm:pli-16'>
        {examSetId ? 'Edit Viva Exam Set' : 'Add Viva Exam Set'}
      </DialogTitle>
...
export default AddEditVivaExamSetsDialog
