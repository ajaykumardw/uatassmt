/*
  ...
  - You are about to drop the `_pctoquestions` table. If the table is not empty, all the data it contains will be lost.
...
-- DropTable
DROP TABLE `_pctoquestions`;

-- CreateTable
CREATE TABLE `pc_quetions` (
...
-- AddForeignKey
ALTER TABLE `pc_quetions` ADD CONSTRAINT `pc_quetions_pc_id_fkey` FOREIGN KEY (`pc_id`) REFERENCES `pc`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `pc_quetions` ADD CONSTRAINT `pc_quetions_question_id_fkey` FOREIGN KEY (`question_id`) REFERENCES `questions`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
