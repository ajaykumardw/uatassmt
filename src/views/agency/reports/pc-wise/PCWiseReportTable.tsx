...
import PCReportDialog from './PCReportDialog';
...
const PCWiseReportTable = () => {
  ...
  const handlePCReportOpen = (candidate: string) => {
  ...
  const columns = useMemo<ColumnDef<StudentsTypeWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('status', {
        header: 'Status',
        cell: ({ row }) =>
        {return absentStudents.includes(row.original.candidate_id.toString()) ? (
          <Chip
            variant='outlined'
      ...
      columnHelper.accessor('action', {
        header: 'Action',
        cell: ({ row }) => {
          const isAbsent = absentStudents.includes(row.original.candidate_id.toString());
            ...
            <div className="flex items-center gap-3">
              {isAbsent ? null : (
                <Button
                  variant="outlined"
                  size="small"
                  onClick={() => handlePCReportOpen(row.original.candidate_id)}
                >
                  View Report
                </Button>
              )}
