...
const TableFilters = ({ setSSCID, setQPID, setPCID, setAllPC, onChange }: { setSSCID?:any, setQPID?:any, setPCID?: any, setAllPC: any, onChange: React.Dispatch<React.SetStateAction<Filters>> }) => {
  // States
  ...
  const handleSSCChange = async (ssc: string) => {
    ...
    const selectedSSC = structure.find(ssc => ssc.id === sscId);
  ...
  const handleQPChange = async (qp: string) => {
    ...
    if (selectedQP) {
      ...
      setAllPC(
        selectedQP.nos?.reduce((acc: PCType[], nos) => {
          if (nos.pc) {
            acc.push(...nos.pc);
          }
  ...
  const handleNOSChange = async (nos: string) => {
    ...
    if (selectedNOS) {
      ...
      // console.log(selectedNOS.pc);
      
      setPCData(selectedNOS.pc || []);
      setAllPC(selectedNOS.pc || []);
    } else {
      setPCData([]);
      setAllPC([]);
    }
