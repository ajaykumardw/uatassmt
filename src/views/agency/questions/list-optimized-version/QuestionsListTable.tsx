...
import type { Filters } from './utils/questionApi';
...
const QuestionsListTable = () => {
  ...
  const [filters,setFilters]=useState<Filters>({
    ...
    search:"",
    page:1,
    limit:TableRowLimit.rowLimit[0] || 20
  });


  // const [selectedIds, setSelectedIds] = useState<number[]>([]);

  const { data: datas, total } = useQuestions(filters)
  ...
  const table = useReactTable({
    ...
    onPaginationChange: (updater) => {
      const newState = typeof updater === 'function' ? updater({pageIndex: filters.page-1, pageSize:filters.limit}) : updater;
  ...
  return (
    <>
      <Card>
        ...
        <TableFilters setSSCID={setSSCID} setQPID={setQPID} setPCID={setPCID} setAllPC={setPCData} onChange={setFilters} />
        <div className='flex justify-between flex-col items-start md:flex-row md:items-center p-6 border-bs gap-4'>
          ...
          <div className='flex flex-col sm:flex-row is-full sm:is-auto items-start sm:items-center gap-4'>
            <DebouncedInput
              ...
              onChange={value => setFilters(prev => ({ ...prev, search: String(value), page: 1 }))}
        ...
        <TablePagination
          component={() => <TablePaginationComponent table={table} total={total} />}
          count={total}
          ...
          rowsPerPageOptions={TableRowLimit.rowLimit}
        />
      </Card>
