...
type TabCategory = 'batches' | 'sales' | 'profit' | 'income'
...
const AssessorReports = ({ serverMode, data }: { serverMode: SystemMode, data: batches[] }) => {
  // States
  const [value, setValue] = useState<TabCategory>('batches')
  ...
  data.forEach(batch => {
    ...
    const date = batch.assessor_assign_datetime ? new Date(batch.assessor_assign_datetime) : new Date();
    const monthIndex = date.getMonth(); // 0 = January, 1 = February, etc.

    monthlyData[monthIndex] += 1; // Increment count for the month
  });
  ...
  return (
    <Card>
      <CardHeader
        title='Batches Reports'
        subheader='Yearly Batches Overview'
...
export default AssessorReports
