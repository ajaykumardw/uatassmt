...
async function generatePDF(page: any, html: string, headerHtml: string) {
...
export const processQuestionPaper = async (
    job: Job
) => {
    ...
    try {
        ...
        if (!batch.theory_exam_set) {
            ...
            throw new Error("Theory exam set not found");
        }
        ...
        for (
            ...
            papersData.push({
                ...
                mapping,
                question_count: mapping.length,
                created_by: dbJob.requested_by || batch.agency.id,
                agency_id: batch.agency.id
            });
        }
        ...
        const papers = await prisma.offline_candidate_papers.findMany({
        ...
        const questionIds = papers.flatMap(
            paper => (paper.mapping as any[]).map(item => item.question_id)
        );
        ...
        const duration =
            durationInMinutes >= 60
                ? `${Math.floor(durationInMinutes / 60)}:${String(durationInMinutes % 60).padStart(2, '0')} ${Math.floor(durationInMinutes / 60) === 1 ? 'hr' : 'hrs'
                }`
                : `${durationInMinutes} ${durationInMinutes === 1 ? 'min' : 'mins'
                }`;
        ...
        for (
            ...
            const pdf = await generatePDF(page, finalHtml, headerHtml);
            ...
            const progress = 50 + Math.floor((processedPapers / papers.length) * 40);
        ...
        try {
            ...
            throw new Error("Error in storing question paper");
